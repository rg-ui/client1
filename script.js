...
function initFormHandling() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Basic validation
        if (typeof validateForm === 'function' && !validateForm(data)) {
            return;
        }
        ...
        try {
            const response = await fetch("https://formsubmit.co/ajax/deepaktheperformer@gmail.com", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                showNotification('✨ Success! Your inquiry has been sent.', 'success');
                form.reset();
            } else {
                showNotification('❌ Something went wrong. Please try again.', 'error');
                console.error('Form execution error:', result);
            }
...
// Float labels for date input
const dateInput = document.getElementById('eventDate');
if (dateInput) {
    dateInput.addEventListener('focus', () => {
        dateInput.parentElement.querySelector('label').style.top = '-10px';
        dateInput.parentElement.querySelector('label').style.fontSize = '0.75rem';
    });
}


function validateForm(data) {
    const { name, email, phone, eventType, eventDate } = data;
